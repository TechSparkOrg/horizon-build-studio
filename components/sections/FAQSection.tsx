    ...
    <section className="bg-off-white py-16 sm:py-28">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12">
        <AnimateOnView>
          ...
          >
            {getVal(content, "ask", t.faq.ask, lang)} <ArrowRight className="size-4" />
          </Link>
          <div className="mt-4">
            <Link
              href="/faq"
            ...
            >
              {getVal(content, "viewAll", t.faq.viewAll, lang)} <ArrowRight className="size-3" />
            </Link>
          </div>
          {(() => {
            const modelUrl = getMedia(content, "stopModelPath", "") || models3d[3]?.url || models3d[0]?.url;
            return modelUrl ? (
