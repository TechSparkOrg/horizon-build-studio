...
export default async function EditModelPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Link href="/admin/models" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-6">
        <ArrowLeft className="size-4" /> Back to Models
      </Link>
      <h1 className="text-xl font-bold mb-6">Edit Model Group</h1>
      <form action={saveModel} className="bg-white p-6 border border-gray-200 max-w-2xl space-y-5">
        ...
        <div>
          <ModelFilesEditor
            initialModels={item.models3d.map((m) => ({ url: m.url, filename: m.filename, type: m.type, label: m.label }))}
          />
        </div>
        <div className="border-t border-gray-200 pt-5">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea name="metaDescription" defaultValue={item.metaDescription} maxLength={200} rows={2} className="w-full px-3 py-2 rounded border border-gray-300 text-sm resize-y min-h-20 max-w-md" placeholder="SEO description" />
            </div>
            <div>
