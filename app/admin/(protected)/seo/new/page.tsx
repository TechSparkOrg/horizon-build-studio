...
export default function NewSeoPage() {
  return (
    <div>
      ...
      <h1 className="text-xl font-bold mb-6">New SEO Entry</h1>
      <form action={saveSeo} className="bg-white p-6 border border-gray-200 max-w-3xl space-y-5">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm mb-1">Slug *</label>
            <input name="slug" className={input} placeholder="e.g. about" required />
          </div>
          <div>
            <label className="block text-sm mb-1">Title</label>
            <input name="title" className={input} placeholder="Page heading" />
          </div>
        </div>
        <div>
          <label className="block text-sm mb-1">Text 1 (English)</label>
          <textarea name="text1En" rows={3} className={`${textarea} min-h-20`} placeholder="Primary text content" />
        </div>
        <div>
          <label className="block text-sm mb-1">Text 1 (Nepali)</label>
          <textarea name="text1Np" rows={3} className={`${textarea} min-h-20`} placeholder="प्राथमिक पाठ सामग्री" />
        </div>
        <div>
          <label className="block text-sm mb-1">Text 2 (English)</label>
          <textarea name="text2En" rows={3} className={`${textarea} min-h-20`} placeholder="Secondary text content" />
        </div>
        <div>
          <label className="block text-sm mb-1">Text 2 (Nepali)</label>
          <textarea name="text2Np" rows={3} className={`${textarea} min-h-20`} placeholder="द्वितीयक पाठ सामग्री" />
        </div>
        <div className="border-t border-gray-200 pt-5">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input name="metaTitle" maxLength={90} className={`${input} max-w-md`} placeholder="SEO title" />
            </div>
            <div>
              <label className="block text-sm mb-1">Meta Description</label>
              <p className="text-xs text-gray-500 mb-1">Keep under 160 characters.</p>
              <textarea name="metaDescription" maxLength={200} rows={2} className={`${textarea} max-w-md min-h-20`} placeholder="SEO description" />
            </div>
            <div>
            ...
            <div>
              ...
              <input name="ogImage" className={`${input} max-w-lg`} placeholder="https://..." />
            </div>
            <div>
