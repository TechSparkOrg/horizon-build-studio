...
export function AboutSection({ content }: { content?: SectionContentMap }) {
  const t = useText();
  const lang = useLang();
  const val = (key: string, fb: string) => getVal(content, key, fb, lang);
  const stats = parseJSONLocale<{ value: string; label: string }[]>(content, "stats", t.about.stats, lang);
  const values = parseJSONLocale<{ title: string; body: string }[]>(content, "values", t.about.values, lang);

  return (
    <section id="about" className="bg-off-white py-16 sm:py-28">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        <AnimateOnView className="max-w-3xl">
          <SectionLabel>{val("aboutLabel", t.about.label)}</SectionLabel>
          <h2 className="mt-3 font-display font-bold text-brand-secondary text-3xl sm:text-4xl lg:text-5xl">
            {val("aboutH2", t.about.h2)}
          </h2>
          <p className="mt-4 text-mid-gray text-lg leading-relaxed">
            {val("aboutSubtitle", t.about.description)}
          </p>
        </AnimateOnView>
        ...
        <div className="mt-16 sm:mt-20 grid lg:grid-cols-2 gap-8 sm:gap-12 items-center">
          <AnimateOnView>
            <div className="rounded-2xl overflow-hidden border border-light-gray shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
              <ModelViewer3D src={content?.aboutModel?.mediaUrl || "/glb/house.glb"} loading="lazy" />
            </div>
          </AnimateOnView>

          <AnimateOnView>
            <div>
              <SectionLabel>{val("storyLabel", t.about.approachLabel)}</SectionLabel>
              <h3 className="mt-2 font-display font-bold text-2xl sm:text-3xl text-brand-secondary">
                {val("storyTitle", t.about.storyTitle)}
              </h3>
              <div className="mt-4 space-y-4 text-mid-gray leading-relaxed">
                <p>{val("story1", t.about.story1)}</p>
                <p>{val("story2", t.about.story2)}</p>
              </div>
            </div>
          </AnimateOnView>
        </div>
        ...
        <div className="mt-16 sm:mt-20">
          <AnimateOnView className="text-center mb-10">
            <SectionLabel>{val("valuesLabel", t.about.valuesLabel)}</SectionLabel>
            <h3 className="mt-2 font-display font-bold text-2xl sm:text-3xl text-brand-secondary">
              {val("valuesTitle", t.about.valuesTitle)}
            </h3>
          </AnimateOnView>
        ...
        <div className="mt-12 text-center">
          ...
          >
            {val("aboutCta", t.about.cta)} <ArrowRight className="size-4" />
          </a>
        </div>
      </div>
    </section>
